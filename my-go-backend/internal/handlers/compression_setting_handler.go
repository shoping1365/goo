...
func (h *CompressionSettingHandler) ListCompressionSettings(w http.ResponseWriter, r *http.Request) {
	...
	case userIDStr != "":
		...
		settings, err = h.Service.GetByUser(r.Context(), uint(userID))
	case mediaIDStr != "":
		mediaID, _ := strconv.ParseUint(mediaIDStr, 10, 64)
		settings, err = h.Service.GetByMedia(r.Context(), uint(mediaID))
	case jobIDStr != "":
		jobID, _ := strconv.ParseUint(jobIDStr, 10, 64)
		settings, err = h.Service.GetByJob(r.Context(), uint(jobID))
	default:
...
func (h *CompressionSettingHandler) GetCompressionSetting(w http.ResponseWriter, r *http.Request) {
	...
	setting, err := h.Service.GetByID(r.Context(), uint(id))
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
