...
const _KEY = '__security_settings__'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (body.rateLimit) {
      ...
      // اعتبارسنجی ساده
      const { admin, public: publicUsers } = body.rateLimit as { admin?: { requestsPerMinute?: number }; public?: { requestsPerMinute?: number } }

      if (admin && publicUsers) {
        // بررسی محدودیت‌های منطقی
        if (admin.requestsPerMinute && publicUsers.requestsPerMinute && admin.requestsPerMinute < publicUsers.requestsPerMinute) {
          return {
            success: false,
