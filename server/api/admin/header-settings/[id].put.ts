...
export default defineEventHandler(async (event): Promise<HeaderResponse> => {
  try {
    ...
    console.log('درخواست به‌روزرسانی هدر:', id, JSON.stringify(body, null, 2))
    ...
    console.log('پاسخ به‌روزرسانی هدر:', responseData)
    ...
    console.error('خطا در به‌روزرسانی هدر:', error)
    ...
    if (errorWithData?.data) {
      throw createError({
        ...
        message: errorWithData.data.message || errorWithData.data.error || 'خطا در به‌روزرسانی هدر',
