...
export default defineEventHandler(async (event) => {
  try {
    ...
    const response = await fetchGo(event, `/api/admin/payment-gateways/stripe/transactions?${queryString}`, {
      method: 'GET',
      headers: {
    ...
    throw createError({
      ...
      message: errorObj.statusMessage || 'خطا در دریافت تراکنش‌های درگاه استرایپ'
    })
  }
}) 