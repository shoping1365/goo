...
export default defineEventHandler(async (event) => {
  try {
    ...
    // دریافت سبد خرید بعدی از دیتابیس
    ...
    const totalCount = await db.query(`
      ...
      WHERE c.user_id = $1 AND ci.deleted_at IS NULL AND ci.is_next_purchase = true
    ...
    let nextCartItems: CartItemResult[] = result.map((row) => ({
      id: row.id,
      cartId: row.cart_id,
    ...
    if (nextCartItems.length === 0) {
      try {
        ...
        const sessionNextCartItems = []
        
        // پیدا کردن همه آیتم‌های "خرید بعدی" در همه session ها
        for (const [_sessionId, cart] of Object.entries(allCarts)) {
          ...
          if (cartData && cartData.items && Array.isArray(cartData.items)) {
            for (const item of cartData.items) {
              // چک کنیم که آیا is_next تنظیم شده و true است
              if (item.is_next === true) {
        ...
        nextCartItems = sessionNextCartItems
      } catch (sessionError) {
        console.error('خطا در خواندن session next cart:', sessionError)
      }
    }
    ...
    return {
      ...
      data: {
        ...
        pagination: {
          page: pageNum,
          limit: limitNum,
          total: nextCartItems.length,
          totalPages: Math.ceil(nextCartItems.length / limitNum)
        }
      }
    }

  } catch (error) {
    console.error('خطا در دریافت سبد خرید بعدی:', error)
    ...
    throw createError({
      statusCode: 500,
      message: 'خطا در دریافت سبد خرید بعدی'
    })
  }
})
