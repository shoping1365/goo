...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    return proxy(event, `${base}/api/admin/widgets/order`, {
      method: 'POST',
      body: JSON.stringify(body)
    })
  } catch (error: unknown) {
    console.error('خطا در به‌روزرسانی ترتیب ویجت‌ها:', error)
    ...
    throw createError({
      ...
      message: errorWithStatus.message || errorWithStatus.statusMessage || 'خطا در به‌روزرسانی ترتیب ویجت‌ها'
    })
  }
})
