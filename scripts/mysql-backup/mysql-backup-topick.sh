...
FTP_BASE_DIR="/home/mohamma2/database/topick"
...
DATABASES=(
  "NAME=app_db
USER=app_db
PASSWORD=vnFb*z.G._sLT(Gj
HOST=localhost
REMOTE_DIR=app_db"

  "NAME=topicksh_backup
USER=topicksh_new
PASSWORD=4H5[(bbV.{E2G(_A
HOST=localhost
REMOTE_DIR=topicksh_backup"

  "NAME=visman_db
USER=visman_user
PASSWORD=sv)PnDJUSc%rN%!L
HOST=localhost
REMOTE_DIR=visman_db"
)
...
# Track per-run results so daemon mode can report accurate totals.
LAST_RUN_SUCCESS_COUNT=0
LAST_RUN_FAILURE_COUNT=0
LAST_RUN_TOTAL_COUNT=0
...
run_all_backups() {
  ...
  if (( PARALLEL_JOBS <= 1 )); then
    for entry in "${DATABASES[@]}"; do
      log "Queued $(parse_entry_field "$entry" NAME || echo "(unknown)") for backup [Slot $current_slot]"
      if run_single_backup "$entry" "$current_slot"; then
  ...
  LAST_RUN_FAILURE_COUNT=$failures
  LAST_RUN_TOTAL_COUNT=$((success_count + failures))
...
run_daemon() {
  ...
  local -i total_success=0
  local -i total_failures=0
  local -i total_backups=0
  local -i cycle=0
  while true; do
    ...
    cycle=$((cycle + 1))
    total_success=$((total_success + LAST_RUN_SUCCESS_COUNT))
    total_failures=$((total_failures + LAST_RUN_FAILURE_COUNT))
    total_backups=$((total_backups + LAST_RUN_TOTAL_COUNT))

    log_and_print "[daemon] Cycle ${cycle}: this_run=${LAST_RUN_TOTAL_COUNT} (success=${LAST_RUN_SUCCESS_COUNT}, failed=${LAST_RUN_FAILURE_COUNT}) | total=${total_backups} (success=${total_success}, failed=${total_failures})"
