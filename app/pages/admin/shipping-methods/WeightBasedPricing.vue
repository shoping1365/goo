<template>
  <div class="weight-based-pricing">
      ...
      <div v-if="enabled" class="card-body">
        ...
        <div class="table-section">
          ...
          <div class="weight-table">
            ...
            <div v-for="(range, index) in weightRanges" :key="index" class="table-row">
              <div class="col">
                <input 
                  ...
                  step="0.1"
                  class="range-input"
                  :placeholder="`0 ${getWeightUnitLabel()}`"
                />
              </div>
              <div class="col">
                <input 
                  ...
                  step="0.1"
                  class="range-input"
                  :placeholder="`${range.from + 1} ${getWeightUnitLabel()}`"
                />
              </div>
              <div class="col">
          ...
          <div class="preview-controls">
            ...
            <div class="preview-input-group">
              <input 
                ...
                placeholder="0"
              />
              <span class="unit-label">{{ getWeightUnitLabel() }}</span>
            </div>
          </div>
          <div class="preview-result">
            <span>قیمت ارسال:</span>
            <strong>{{ calculatePrice(sampleWeight) }} تومان</strong>
          </div>
        </div>
      </div>
    </div>
...
const weightUnit = ref(props.modelValue.weightUnit)
const weightRanges = ref(props.modelValue.weightRanges.length ? props.modelValue.weightRanges : [
  { from: 0, to: 1, price: 50000 },
  { from: 1, to: 5, price: 80000 }
])
const sampleWeight = ref(2)

watch([enabled, weightUnit, weightRanges], () => {
...
function addWeightRange() {
  const lastRange = weightRanges.value[weightRanges.value.length - 1]
  const newFrom = lastRange ? lastRange.to : 0
  weightRanges.value.push({
    from: newFrom,
    to: newFrom + 1,
    price: lastRange ? lastRange.price + 10000 : 50000
  })
}

function removeWeightRange(index) {
  if (weightRanges.value.length > 1) {
    weightRanges.value.splice(index, 1)
  }
}
...
function getWeightUnitLabel() {
  ...
  return labels[weightUnit.value] || weightUnit.value
}

function calculatePrice(weight) {
  if (!weight || weight <= 0) return 0
  
  const range = weightRanges.value.find(r => 
    weight >= r.from && weight <= r.to
  )
