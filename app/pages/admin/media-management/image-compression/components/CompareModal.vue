    ...
    <div class="relative bg-white rounded-lg overflow-hidden shadow-lg w-[95vw] max-w-none max-h-[95vh] overflow-auto" @click.stop>
      ...
      <div class="flex flex-col sm:flex-row">
        ...
        <div class="flex-1 p-6 flex flex-col items-center border-b sm:border-b-0 sm:border-r">
          ...
          <span class="text-sm text-gray-600 mt-2" v-if="compareModalData">{{ formatFileSize(compareModalData.originalSize) }}</span>
        </div>
        <!-- Compressed -->
        <div class="flex-1 p-6 flex flex-col items-center">
          ...
          <span class="text-sm text-gray-600 mt-2" v-if="compareModalData">{{ formatFileSize(compareModalData.compressedSize) }}</span>
        </div>
      </div>
    </div>
  </div>
