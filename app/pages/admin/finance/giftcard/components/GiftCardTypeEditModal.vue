  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <form class="p-6 space-y-6" @submit.prevent="handleSubmit">
        ...
        <!-- آمار کارت‌های موجود -->
        <div class="border-t border-gray-200 pt-6">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">آمار کارت‌های موجود</h3>
          
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-blue-50 rounded-lg p-6 text-center">
              <div class="text-2xl font-bold text-blue-600">{{ existingCardsStats.total }}</div>
              <div class="text-sm text-blue-600">کل کارت‌ها</div>
            </div>
            <div class="bg-green-50 rounded-lg p-6 text-center">
              <div class="text-2xl font-bold text-green-600">{{ existingCardsStats.active }}</div>
              <div class="text-sm text-green-600">کارت‌های فعال</div>
            </div>
            <div class="bg-orange-50 rounded-lg p-6 text-center">
              <div class="text-2xl font-bold text-orange-600">{{ formatPrice(existingCardsStats.totalValue) }}</div>
              <div class="text-sm text-orange-600">کل ارزش</div>
            </div>
          </div>
        </div>
...
const emit = defineEmits<{
  close: []
  updated: [giftCardType: GiftCardType]
}>()
...
watch(() => props.type, (newType) => {
  if (newType) {
    form.value = {
      ...
      minAmount: newType.minAmount?.toString() || '',
      maxAmount: newType.maxAmount?.toString() || '',
      fixedAmount: newType.fixedAmount?.toString() || '',
      discountPercentage: newType.discountPercentage?.toString() || '',
      themes: newType.themes || [],
      isActive: newType.isActive,
      showInStore: newType.showInStore,
      requiresApproval: newType.requiresApproval
    }
    
    // بارگذاری آمار کارت‌های موجود
    loadExistingCardsStats(newType.id)
  }
}, { immediate: true })
...
const loadExistingCardsStats = async (_typeId: string) => {
  try {
    ...
    // داده‌های نمونه
    existingCardsStats.value = {
      total: 156,
      active: 142,
      totalValue: 78000000
    }
  } catch (error) {
    console.error('خطا در بارگذاری آمار کارت‌ها:', error)
  }
}

const handleSubmit = async () => {
  if (isSubmitting.value || !props.type) return
  ...
  try {
    ...
    const updatedType: GiftCardType = {
      ...props.type,
    ...
    console.error('خطا در ویرایش نوع گیفت کارت:', error)
  } finally {
