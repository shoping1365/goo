  ...
  <div class="bg-white rounded-lg shadow-lg p-6">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-3 space-x-reverse">
        ...
        <div>
          <h3 class="text-lg font-semibold text-gray-900">تراکنش‌های درگاه پارسیان</h3>
          <p class="text-sm text-gray-500">مشاهده و مدیریت تراکنش‌های پرداخت بانک پارسیان</p>
        </div>
      </div>
...
const fetchTransactions = async () => {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse>(`/api/admin/payment-gateways/parsian.transactions?${params}`)
    ...
    message.value = 'خطا در دریافت تراکنش‌های درگاه پارسیان'
    messageType.value = 'error'
  } finally {
    loading.value = false
  }
...
const refundTransaction = async (transaction: Transaction) => {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse>('/api/admin/payment-gateways/parsian.refund', {
      method: 'POST',
      body: {
