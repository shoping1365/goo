<template>
  <div class="bg-white rounded-lg shadow-lg p-6">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center space-x-3 space-x-reverse">
        <div class="w-12 h-12 bg-purple-600 rounded-lg flex items-center justify-center">
          <span class="text-white font-bold text-lg">پارسیان</span>
        </div>
        <div>
          <h3 class="text-lg font-semibold text-gray-900">تنظیمات درگاه پارسیان</h3>
          <p class="text-sm text-gray-500">مدیریت تنظیمات و پارامترهای درگاه پرداخت بانک پارسیان</p>
        </div>
      </div>
    ...
    <div v-if="loading" class="flex justify-center py-8">
      <div class="i-heroicons-arrow-path animate-spin text-2xl text-purple-600"></div>
    </div>
    ...
    <form v-else @submit.prevent="saveSettings" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">شماره پذیرنده</label>
          <input
            v-model="settings.merchant_id"
            type="text"
            required
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            placeholder="شماره پذیرنده پارسیان"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            required
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            placeholder="کلید خصوصی پارسیان"
          />
        </div>
      </div>
      ...
      <div class="bg-gray-50 rounded-lg p-6">
        <h4 class="text-sm font-medium text-gray-900 mb-3">اطلاعات درگاه پارسیان</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
          <div>
            <span class="text-gray-600">نوع درگاه:</span>
            <span class="mr-2 font-medium">پارسیان (PEC)</span>
          </div>
          <div>
...
const fetchSettings = async () => {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse>('/api/admin/payment-gateways/parsian.settings')
    ...
    message.value = 'خطا در دریافت تنظیمات درگاه پارسیان'
...
const saveSettings = async () => {
  ...
  try {
    ...
    if (response.success) {
      message.value = 'تنظیمات درگاه پارسیان با موفقیت ذخیره شد'
    ...
    message.value = 'خطا در ذخیره تنظیمات درگاه پارسیان'
    messageType.value = 'error'
  } finally {
    saving.value = false
  }
...
const testConnection = async () => {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse>('/api/admin/payment-gateways/parsian.test-connection', {
      method: 'POST'
    })
    
    if (response.success) {
      message.value = 'اتصال به درگاه پارسیان موفقیت‌آمیز بود'
      messageType.value = 'success'
    } else {
    ...
    message.value = 'خطا در تست اتصال به درگاه پارسیان'
    messageType.value = 'error'
  } finally {
    testing.value = false
  }
