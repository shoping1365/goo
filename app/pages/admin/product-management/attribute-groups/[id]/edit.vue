  ...
  try {
    ...
    if (Array.isArray(g.attributes)) {
      attributes.value = g.attributes.map((rec: any) => ({
        ...
        type: faType(rec.type || 'انتخاب'),
        control_type: faControl(rec.control_type) || (faType(rec.type || 'انتخاب') === 'انتخاب'
          ? 'لیست باز شونده تک انتخابی'
          : 'تکست باکس متنی تک خطی'),
        has_filter: rec.has_filter || false,
        is_key: rec.is_key || false,
        show_on_product: rec.show_on_product ?? true,
...
// Attributes list for this group (fetched from backend)
...
const deleteGroup = async () => {
  ...
  const ok = await confirm({ title:'تأیید حذف', message:'آیا از حذف این گروه اطمینان دارید؟', confirmText:'حذف', cancelText:'انصراف', type:'danger' })
  if (ok) router.push('/admin/attribute-groups')
}
...
const filteredModalAttributes = computed(() => {
  if (!modalSearch.value) return availableAttributes.value
  return availableAttributes.value.filter(a => a.name.includes(modalSearch.value))
})
...
const isAlreadySelected = (attr:{id:number,name:string})=>selectedNames.value.includes(attr.name)
