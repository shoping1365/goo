  ...
  <div class="product-new-page min-h-screen p-6">
    ...
    <div class="flex items-center justify-between bg-gray-50 px-6 py-4 mb-4 rounded-t shadow-sm">
      <div class="flex items-center gap-6">
        <h1 class="text-lg font-bold">افزودن محصول جدید</h1>
      </div>
      <div class="flex items-center gap-6">
     ...
     <div class="tab-content">
       <ClientOnly>
         <component 
           ...
           v-bind="activeTab === 'images' ? { defaultMediaCategory: 'products' } : {}" 
         />
       </ClientOnly>
     </div>

    <!-- Global Confirm Dialog for this page (provided to children) -->
...
// همیشه در صفحه «افزودن محصول جدید» فرم را صفر می‌کنیم تا دادهٔ قبلی نمایش داده نشود
pStore.$reset()
...
const tabComponents = {
  ...
  images: defineAsyncComponent(() => import('~/components/product/ProductImagesTab.vue')),
  ...
  seo: defineAsyncComponent(() => import('~/components/product/ProductSeoTab.vue')),
  complements: defineAsyncComponent(() => import('./complements.vue')),
  video: defineAsyncComponent(() => import('~/components/product/ProductVideoTab.vue'))
}

const currentTabComponent = computed(() => {
  const tabKey = activeTab.value
  return tabComponents[tabKey] || tabComponents.info
})

const setActiveTab = (tabValue: string) => {
  if (typeof tabValue === 'string' && tabComponents[tabValue]) {
    activeTab.value = tabValue
  }
}

// Whenever the active tab changes, sync it back to the url so users can copy/share the link
...
onMounted(async () => {
  ...
  // این صفحه همیشه باید خالی باشد؛ هرگونه شناسهٔ قبلی را حذف می‌کنیم
  // فرانت حق نگهداری شناسه ندارد

  // بارگذاری برای محصول جدید
  pStore.loadCategories()
  loadBrands()

  // Set the default tab
  const queryTab = route.query.tab
  activeTab.value = typeof queryTab === 'string' && tabComponents[queryTab] ? queryTab : 'info'
})
...
onUnmounted(() => {
  ...
  if (typeof window !== 'undefined') {
    ...
    localStorage.removeItem('editingProductSavedAt')
  }
...
async function saveProduct() {
  try {
    ...
    // جلوگیری از دوبل‌توست: اگر خطا از fetch آمده، اینترسپتور نمایش می‌دهد.
    if (error?.response || error?.skipToast) return
    const msg = error?.message || 'خطا در ذخیره محصول'
    notifier.error(msg, 'خطا در ذخیره محصول')
  }
}
...
async function previewProduct() {
  try {
    ...
    // Error previewing product
    if (error?.response || error?.skipToast) return
    const msg = error?.message || 'خطا در پیش‌نمایش محصول'
    notifier.error(msg, 'خطا در پیش‌نمایش محصول')
  }
}
...
function saveSectionSettings() {
  if (typeof window !== 'undefined') {
    localStorage.setItem('productPageSections', JSON.stringify(sectionSettings.value))
  }
}

// Watch for section settings changes and save them
watch(sectionSettings, () => {
  saveSectionSettings()
}, { deep: true })
    ...
    const app = createApp(mod.default, {
      ...
      'onSections-updated': (newSettings) => {
        ...
        if (!availableTabs.includes(activeTab.value)) {
          ...
          if (firstAvailableTab && tabComponents[firstAvailableTab]) {
...
// Provide product name as pageTitle for SEO tab
