...
<template>
  <!-- تب اطلاع‌رسانی شده تخفیف -->
  <div v-if="props.activeNavItem === 'discount-completed'" class="w-full">
    <!-- فیلترها و عملیات -->
    <div v-if="selectedDiscountCompletedItems.length > 0" class="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-6">
      <div class="flex flex-row-reverse items-center gap-3">
        ...
        >
          ...
          <span>اطلاع‌رسانی مجدد ({{ selectedDiscountCompletedItems.length }})</span>
        </TemplateButton>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
      <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900">اطلاع‌رسانی‌های موفق تخفیف</h3>
        <p class="text-sm text-gray-600 mt-1">{{ discountCompletedRequests.length }} مورد</p>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tr class="text-right" dir="rtl">
            ...
            <th class="text-right py-3 px-4 text-sm font-medium text-purple-900" style="text-align: right;">تاریخ اطلاع‌رسانی</th>
            <th class="text-right py-3 px-4 text-sm font-medium text-purple-900" style="text-align: right;">قیمت</th>
            <th class="text-right py-3 px-4 text-sm font-medium text-purple-900" style="text-align: right;">متقاضی</th>
            <th class="text-right py-3 px-4 text-sm font-medium text-purple-900" style="text-align: right;">وضعیت</th>
          </tr>
          </thead>
          ...
          <tr v-if="discountCompletedRequests.length === 0">
