<template>
  <div class="min-h-screen">
    ...
    <div class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
      <div class="px-6 py-4">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">افزودن نوشته جدید</h1>
            <p class="text-sm text-gray-500 mt-1">ایجاد و انتشار محتوای جدید</p>
          </div>
    ...
    <div class="p-6 w-full">
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 w-full items-start">
        <!-- فرم اصلی -->
        <div class="lg:col-span-4 space-y-6 w-full">
          ...
          <!-- خلاصه نوشته -->
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 w-full mt-0">
...
useHead({
  title: 'افزودن نوشته جدید - پنل مدیریت'
})
...
// ذخیره نوشته به صورت پیش‌نویس برای پیش‌نمایش
const saveAsDraft = async () => {
  ...
  try {
    // آماده‌سازی داده‌ها برای ارسال به صورت پیش‌نویس
    const postData = {
      title: postForm.title,
      excerpt: postForm.excerpt,
      content: postForm.content,
      status: 'draft', // همیشه به صورت پیش‌نویس ذخیره می‌شود
      ...
      tags: postForm.tags,
      language: postForm.language,
      is_accessible_for_free: postForm.is_accessible_for_free,
    }
    ...
    if (response && response.id) {
      // ذخیره ID نوشته برای استفاده در مدیریت عناوین
      postForm.id = response.id
      return response // برگرداندن پاسخ برای استفاده در پیش‌نمایش
    }
    return false
  } catch (error) {
...
const previewPost = async () => {
  try {
    // ابتدا نوشته را به صورت پیش‌نویس ذخیره کن
    const savedPost = await saveAsDraft()
    
    if (savedPost && savedPost.id) {
      // اگر ذخیره موفق بود، پیش‌نمایش را باز کن
      if (postForm.slug && getCategorySlug(postForm.category_id)) {
        const previewUrl = `/blog/${getCategorySlug(postForm.category_id)}/${postForm.slug}`
        window.open(previewUrl, '_blank', 'noopener,noreferrer')
      } else if (postForm.slug) {
        const previewUrl = `/blog/${postForm.slug}`
        window.open(previewUrl, '_blank', 'noopener,noreferrer')
      }
    }
  } catch (error) {
...
const saveAndContinueEditing = async () => {
  try {
    ...
    if (response && response.id) {
      ...
      // ذخیره ID در localStorage برای مدیریت refresh صفحه
      if (typeof window !== 'undefined') {
        localStorage.setItem('editingPostId', response.id.toString())
      }
      
      // هدایت به صفحه ویرایش همان نوشته
      navigateTo(`/admin/post-management/edit-post/${response.id}`)
    }
  } catch (error) {
...
// Methods for title management
// insertTitleInContent method is removed as TitleSelector is removed

// handleHTagSelected method is removed as TitleSelector is removed

// handleTitleSelected method is removed as TitleSelector is removed
...
onMounted(async () => {
  ...
  // بررسی وجود محتوای تولید شده توسط AI
  if (typeof window !== 'undefined') {
    const aiGeneratedContent = localStorage.getItem('aiGeneratedContent')
    if (aiGeneratedContent) {
      try {
        const content = JSON.parse(aiGeneratedContent)
        fillFormWithAIContent(content)
        localStorage.removeItem('aiGeneratedContent') // پاک کردن از localStorage
      } catch (error) {
        console.error('خطا در بارگذاری محتوای AI:', error)
      }
    }
  }
})
...
const fillFormWithAIContent = (content: any) => {
  ...
  if (content.excerpt) {
    postForm.excerpt = content.excerpt
  }
  
  if (content.content) {
    postForm.content = content.content
  }
  ...
  // نمایش پیام موفقیت
  alert('محتوای تولید شده توسط AI با موفقیت بارگذاری شد!')
}
</script> 
