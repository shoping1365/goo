  ...
  <div class="w-full">
    ...
    <div class="bg-white rounded-xl shadow p-8 mt-6 mx-4">
      ...
      <DeviceTabs
        ...
        :current-preview-slide="currentPreviewBanner"
...
useHead({ title: 'ویرایش بنر دوتایی - پنل ادمین' })
...
const initializeFormData = () => {
  if (widget.value) {
    ...
    formData.value = {
      ...
      type: widget.value.type || 'double-banner',
...
const widgetType = computed(() => widget.value?.type || 'double-banner')
...
const bannerConfig = ref<BannerConfig>({
  ...
  mobile_slider_width: 300, // عرض اسلایدر در موبایل
...
const openAddBannerModal = () => {
  ...
  // تشخیص آرایه فعال بر اساس تب
  const activeBannersArray = activeDeviceTab.value === 'mobile' 
  ...
  editingBanner.value = {
    ...
    image: '',
    link: '',
    order: activeBannersArray.length + 1,
    status: 'active'
  }
...
const editBanner = (index: number) => {
  ...
  const banner = activeBannersArray[index]
  editingBanner.value = {
    title: banner.title || '',
...
const handleBannerSave = (bannerData: BannerItem, showTitle: boolean) => {
  ...
  const banner: BannerItem = {
    ...
    order: editingBannerIndex.value !== null ? activeBannersArray[editingBannerIndex.value].order : activeBannersArray.length + 1,
    status: 'active',
    showTitle: showTitle
  }
  ...
  if (editingBannerIndex.value !== null) {
    // ویرایش بنر موجود
    activeBannersArray[editingBannerIndex.value] = banner
  } else {
    // افزودن بنر جدید
    activeBannersArray.push(banner)
  }
...
const updateBanner2Ratio = () => {
  bannerConfig.value.banner2_ratio = 100 - bannerConfig.value.banner1_ratio
}

const updateBanner1Ratio = () => {
  bannerConfig.value.banner1_ratio = 100 - bannerConfig.value.banner2_ratio
}
...
onMounted(async () => {
  ...
  if (widget.value?.config) {
    ...
    if (config.mobile_banners) bannerConfig.value.mobile_banners = config.mobile_banners  // اضافه کردن mobile_banners
    ...
    if (config.mobile_slider_width !== undefined) bannerConfig.value.mobile_slider_width = config.mobile_slider_width
    ...
    // نسبت بنرها
    if (config.banner1_ratio !== undefined) bannerConfig.value.banner1_ratio = config.banner1_ratio
    if (config.banner2_ratio !== undefined) bannerConfig.value.banner2_ratio = config.banner2_ratio
  }
})
</script>