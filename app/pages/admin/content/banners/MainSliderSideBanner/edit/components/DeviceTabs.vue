      ...
      <div class="bg-white rounded-xl shadow p-8 mt-6 mx-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
          ...
          <div>
            ...
            <input
              type="number"
              :value="localSliderConfig.mobile_padding_top !== undefined ? localSliderConfig.mobile_padding_top : ''"
              min="0"
              max="100"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
              placeholder="0"
              @input="e => localSliderConfig.mobile_padding_top = (e.target as HTMLInputElement).value === '' ? undefined : Number((e.target as HTMLInputElement).value)"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              :value="localSliderConfig.mobile_padding_bottom !== undefined ? localSliderConfig.mobile_padding_bottom : ''"
              min="0"
              max="100"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
              placeholder="0"
              @input="e => localSliderConfig.mobile_padding_bottom = (e.target as HTMLInputElement).value === '' ? undefined : Number((e.target as HTMLInputElement).value)"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              :value="localSliderConfig.mobile_margin_right !== undefined ? localSliderConfig.mobile_margin_right : ''"
              min="0"
              max="100"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
              placeholder="0"
              @input="e => localSliderConfig.mobile_margin_right = (e.target as HTMLInputElement).value === '' ? undefined : Number((e.target as HTMLInputElement).value)"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              :value="localSliderConfig.mobile_margin_left !== undefined ? localSliderConfig.mobile_margin_left : ''"
              min="0"
              max="100"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
              placeholder="0"
              @input="e => localSliderConfig.mobile_margin_left = (e.target as HTMLInputElement).value === '' ? undefined : Number((e.target as HTMLInputElement).value)"
            />
          </div>
          ...
          <div>
            ...
            <select
              v-model="localSliderConfig.mobile_autoplay_enabled"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
          ...
          <div>
            ...
            <select
              v-model="localSliderConfig.mobile_autoplay_delay"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
          ...
          <div>
            ...
            <select
              v-model="localSliderConfig.mobile_loop_enabled"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
              <option :value="true">فعال</option>
              <option :value="false">غیرفعال</option>
          ...
          <div>
            ...
            <select
              v-model="localSliderConfig.mobile_show_navigation"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
          ...
          <div>
            ...
            <select
              v-model="localSliderConfig.mobile_show_pagination"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
              <option :value="true">نمایش</option>
              <option :value="false">مخفی</option>
          ...
          <!-- رنگ پس‌زمینه -->
                    ...
                    <div>
                      ...
                      <select
                        v-model="localSliderConfig.mobile_banner_width"
                    ...
                    <select
                         v-model="localSliderConfig.mobile_height"
                         class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
                    >
                         <option value="auto">اتوماتیک</option>
                    ...
                    <select
                         v-model="localSliderConfig.mobile_banner_height"
                         class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400"
                    >
                         <option value="auto">اتوماتیک</option>
                         <option :value="100">100 پیکسل</option>
                    ...
                    <div class="flex gap-6 mt-4 w-full">
                                    <!-- پیش‌نمایش زنده -->
                                    <div class="rounded-xl shadow p-2 ml-auto" :style="mobilePreviewWrapperStyle">
                                    ...
                                    <!-- Layout container for mobile - vertical stack -->
                                                  <div
                                        class="flex flex-col gap-3 p-2 min-h-[500px] relative mx-auto w-full"
                                        :style="mobilePreviewInnerStyle"
                                    >
                                                  <!-- Slider Section - Full width for mobile -->
                                                  <div
                                        class="relative overflow-hidden rounded-lg w-full"
                                                        :style="mobileSliderContainerStyle"
                                    >
                  <!-- نمایش اسلایدها -->
                  ...
                  <div v-else class="flex items-center justify-center h-full text-gray-400">
                    ...
                    <p class="text-xs text-gray-500">برای فعال شدن پیش‌نمایش موبایل، برای هر اسلاید تصویر موبایل بارگذاری کنید</p>
                                                  ...
                                                  <div
                                        class="relative overflow-hidden rounded-lg w-full"
                            :style="mobileBannerContainerStyle"
                                    >
...
// Ensure select inputs have sensible defaults even if backend omits them
const hasInitialized = ref(false)

const ensureAutoDefaults = () => {
  if (!localSliderConfig.value) {
    return
  }

  const keys: Array<'mobile_slider_width' | 'mobile_height' | 'mobile_banner_height' | 'mobile_banner_width'> = [
    'mobile_slider_width',
    'mobile_height',
...
// Tab state
          ...
          const DEFAULT_PREVIEW_MAX_WIDTH = 420
          const FALLBACK_MOBILE_SECTION_HEIGHT = 150
          ...
          const mobilePreviewWrapperStyle = computed<CSSProperties>(() => {
            const sliderWidth = resolveNumeric(props.sliderConfig?.mobile_slider_width)
            const bannerWidth = resolveNumeric(props.sliderConfig?.mobile_banner_width)
            const widths = [sliderWidth, bannerWidth].filter((w): w is number => w !== null)
            if (widths.length === 0) {
              return {
                width: '100%',
                maxWidth: `${DEFAULT_PREVIEW_MAX_WIDTH}px`
              }
            }

            const maxWidth = Math.min(Math.max(...widths), DEFAULT_PREVIEW_MAX_WIDTH)
            return {
              width: '100%',
          ...
          const mobilePreviewInnerStyle = computed<CSSProperties>(() => ({
            width: '100%',
            maxWidth: mobilePreviewWrapperStyle.value.maxWidth
          }))

          const resolvedMobileSliderWidth = computed(() => (
          ...
          const mobileSliderContainerStyle = computed<CSSProperties>(() => {
            ...
            const height = resolveNumeric(heightValue) ?? FALLBACK_MOBILE_SECTION_HEIGHT
            const width = resolvedMobileSliderWidth.value
          ...
          const mobileBannerContainerStyle = computed<CSSProperties>(() => {
            ...
            const height = resolveNumeric(heightValue)
            const resolved = height ?? FALLBACK_MOBILE_SECTION_HEIGHT
          ...
          const mobileBannerItemStyle = computed<CSSProperties>(() => {
            ...
            const perItem = mobileBanners.value.length > 0 ? available / mobileBanners.value.length : totalHeight

            return {
              height: `${Math.max(perItem, 0)}px`
            }
          })
