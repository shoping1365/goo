<template>
  <!-- اعلان‌ها -->
  <div :style="widgetStyle" class="notification-widget-container">
    <NuxtLink to="/notifications" class="relative flex items-center text-xl w-full h-full" :class="getLinkAlignment()">
      🔔
      <span v-if="count && count > 0" class="absolute -top-1 -right-2 text-xs bg-orange-500 text-white rounded-full px-1">{{ count }}</span>
    </NuxtLink>
  </div>
</template>
