...
interface HeaderLayer {
  ...
  style_settings?: string | Record<string, unknown>;
  [key: string]: unknown;
}

const newLayer = inject<Ref<HeaderLayer>>('newLayer', ref({
  id: '',
  name: '',
  width: 100,
  height: 50,
...
const createdLayers = inject<Ref<HeaderLayer[]>>('createdLayers', ref([]))
...
function parseStyleSettings(layer: HeaderLayer | Record<string, unknown>) {
...
function resolveBorderConfig(layer: HeaderLayer | Record<string, unknown>) {
...
function resolveShadowConfig(layer: HeaderLayer | Record<string, unknown>) {
  const styleSettings = parseStyleSettings(layer) || {}
...
function getPreviewLayerStyle(layer: HeaderLayer, index: number) {
  ...
  const base: Record<string, string | number> = {
    backgroundColor: (layer?.color as string) || 'transparent',
...
function startResize(itemIndex: number, event: MouseEvent | TouchEvent) {
  ...
  initialWidths.value = newLayer.value.items.map((item: HeaderItem) => {
...
function hasItem(obj: HeaderItem | string | Record<string, unknown>, id: string) {
  if (!obj) return false

  if (typeof obj === 'object' && 'id' in obj && (obj as Record<string, unknown>).id === id) {
    return true
  }

  if (typeof obj === 'object' && 'items' in obj && Array.isArray((obj as Record<string, unknown>).items)) {
    const items = (obj as Record<string, unknown>).items as string[]
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    if ((obj as Record<string, unknown>)[id] !== undefined) {
      return true
    }
  }
  return false
...
function onMediaSelected(selectedMedia: Record<string, unknown>[]) {
  try {
    if (selectedMedia.length > 0 && activeIndex.value !== null) {
      ...
      if (currentItem && currentItem.id === 'image') {
        ...
        currentItem.imageUrl = media.url as string
        currentItem.imageName = media.name as string
        currentItem.imageId = Number(media.id)
    ...
    alert('خطا در انتخاب عکس: ' + (error as Error).message)
  }
}
