<template>
  <div class="create-mobile-app-header-page">
    <div class="header-bg">
      <div class="page-header-flex">
        <div>
          <h1 class="page-title">ایجاد هدر موبایل و اپلیکیشن</h1>
          <p class="page-subtitle">هدر جدید برای موبایل و اپلیکیشن ایجاد کنید</p>
        </div>
    ...
    <div class="templates-section">
      ...
      <div class="templates-grid">
        <div class="template-card" @click="selectTemplate('template1')">
        ...
        <div class="template-card" @click="selectTemplate('template2')">
          <div class="template-preview">
            <div class="mobile-header-preview">
        ...
        <div class="template-card" @click="selectTemplate('template3')">
          <div class="template-preview">
            <div class="mobile-header-preview">
        ...
        <form @submit.prevent="createMobileAppHeader" class="mobile-app-header-form">
          ...
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <select id="pageSelection" v-model="formData.page_selection" class="form-select">
                <option value="all">همه صفحات</option>
                <option value="specific">صفحات خاص</option>
                <option value="exclude">مستثنی کردن صفحات</option>
              </select>
            </div>
            <div v-if="formData.page_selection === 'specific'" class="form-group">
            ...
            <div v-if="formData.page_selection === 'exclude'" class="form-group">
            ...
            <div class="form-actions">
              ...
              <button type="submit" class="btn btn-primary" :disabled="loading">
                ...
                <span v-else>ایجاد هدر موبایل و اپلیکیشن</span>
              </button>
            </div>
          </div>
        </form>
            ...
            <div class="live-preview-mobile">
              <div class="live-preview-screen">
                ...
                <div v-if="selectedTemplate === 'template1'" class="live-preview-header" :style="{ backgroundColor: formData.background_color, color: formData.text_color }">
                  <!-- Debug: نمایش مقادیر عکس -->
                  <div v-if="formData.top_image_url || formData.bottom_image_url" style="background: yellow; padding: 4px; font-size: 10px;">
                    Debug: top_image_url = {{ formData.top_image_url }}, bottom_image_url = {{ formData.bottom_image_url }}
                  </div>
                  
                  <!-- تست: نمایش همیشه -->
                  <div style="background: red; padding: 4px; font-size: 10px; color: white;">
                    تست: همیشه نمایش داده می‌شود
                  </div>
                  ...
                  <!-- تست: نمایش عکس بدون شرط -->
                  <div style="background: blue; padding: 4px; font-size: 10px; color: white;">
                    تست عکس: {{ formData.top_image_url ? 'عکس موجود است' : 'عکس موجود نیست' }}
                  </div>
                  
                  <div class="live-preview-top-bar">
                    <div class="live-preview-logo">{{ formData.name || 'نام هدر' }}</div>
                    <div class="live-preview-icons">
                      <div class="live-preview-icon moon-icon">🌙</div>
                      <div class="live-preview-icon user-icon">👤</div>
                    </div>
                  </div>
                  <div class="live-preview-search-section">
                    <div class="live-preview-search-bar">
                      <div class="live-preview-search-icon">🔍</div>
                      <span class="live-preview-search-text">{{ formData.name || 'نام سایت' }}</span>
                    </div>
                    <div class="live-preview-cart-icon">🛒</div>
                  </div>
              ...
              <div v-else-if="selectedTemplate === 'template2'" class="live-preview-header" :style="{ backgroundColor: formData.background_color, color: formData.text_color }">
                ...
                <div class="live-preview-banner-section">
                  <div class="live-preview-banner">
                    <div class="live-preview-banner-content">
                      <span class="live-preview-banner-text">{{ formData.description || 'توضیحات هدر' }}</span>
                    </div>
                    <div class="live-preview-paperclip">📎</div>
                  </div>
                </div>
                <div class="live-preview-search-section-2">
                  <div class="live-preview-search-bar-2">
                    <span class="live-preview-search-text-2">{{ formData.name || 'نام سایت' }}</span>
                    <div class="live-preview-logo-left-2">{{ formData.name || 'نام هدر' }}</div>
                  </div>
                  <div class="live-preview-notification-icon">🔔</div>
                </div>
              ...
              <div v-else-if="selectedTemplate === 'template3'" class="live-preview-header" :style="{ backgroundColor: formData.background_color, color: formData.text_color }">
                ...
                <div class="live-preview-search-section-3">
                  <div class="live-preview-search-bar-3">
                    <div class="live-preview-search-icon-3">🔍</div>
                    <span class="live-preview-search-text-3">{{ formData.name || 'نام سایت' }}</span>
                    <div class="live-preview-logo-left">
                      <span class="live-preview-logo-text">{{ formData.name || 'نام هدر' }}</span>
                    </div>
                  </div>
                </div>
...
const createMobileAppHeader = async () => {
  loading.value = true
