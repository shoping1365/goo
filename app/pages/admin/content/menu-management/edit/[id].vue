  ...
  <div class="min-h-screen">
    ...
    <div class="header-bg">
      <div class="page-header-flex">
        <h1 class="page-title">ویرایش منو</h1>
        <button class="btn btn-primary new-item-btn" @click="goToMenuList">بازگشت به لیست منوها</button>
      </div>
    </div>
...
const saveCurrentMenu = async () => {
  ...
  try {
    ...
    const successMessage = response?.message ?? "منو با موفقیت به‌روزرسانی شد"
    showSuccess(successMessage, 4000)
  } catch (error) {
    console.error("Error saving menu:", error)
    const errorMessage = error?.data?.message ?? error?.message ?? "خطا در به‌روزرسانی منو"
    showError(errorMessage, 5000)
  }
}

const goToMenuList = () => navigateTo("/admin/content/menu-management")
